...
final class MySqlTableNames
{
    ...
    public static function fromPrefix(string $tablePrefix): self
    {
        if (!preg_match('/^[A-Za-z0-9_]*$/', $tablePrefix)) {
            throw new \InvalidArgumentException('The MySQL table prefix must contain only letters, numbers, and underscores.');
        }
    ...
    private static function validateIdentifier(string $identifier, string $name): void
    {
        if (!preg_match('/^[A-Za-z0-9_]+$/', $identifier)) {
            throw new \InvalidArgumentException(sprintf(
                'The MySQL table name for %s must contain only letters, numbers, and underscores.',
                $name,
            ));
        }
    }
